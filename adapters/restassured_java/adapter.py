...
Executes REST API tests using RestAssured with TestNG or JUnit 5 framework.
"""
...
class RestAssuredJavaAdapter(BaseTestAdapter):
    """Adapter for RestAssured + Java API tests (TestNG or JUnit 5)."""
    
    def __init__(self, project_root: str, config: Optional[RestAssuredConfig] = None):
        """
        Initialize RestAssured + Java adapter.
        ...
        if not self.config.test_framework:
            config_info = self.detector.get_config_info(project_root)
            self.config.test_framework = config_info.get('test_framework', 'testng')
